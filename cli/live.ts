...
import { liveSettingsPath } from "../bridge/live/config.ts";
...
export async function cmdLiveOn(
  ...
  const payload = JSON.stringify({ voice, codexBin: which }, null, 2) + "\n";
  try {
  ...
  deps.io.out(
    `✓ enabled live call (voice: ${voice}) — written to ${path} (no restart needed).`,
  );
  return EXIT.OK;
}
